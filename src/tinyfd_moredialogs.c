...
/* {"Row1 Col1","Row1 Col2","Row2 Col1","Row2 Col2"} */
{
    ...
    if (tfd_quoteDetected(aTitle))
        return tinyfd_arrayDialog("INVALID TITLE WITH QUOTES", aNumOfColumns, aColumns, aNumOfRows, aCells);
    for (i = 0; i < aNumOfColumns; i++)
    {
        if (tfd_quoteDetected(aColumns[i]))
            return tinyfd_arrayDialog("INVALID COLUMNS WITH QUOTES", 0, NULL, 0, NULL);
    }
    for (i = 0; i < aNumOfRows; i++)
    {
        if (tfd_quoteDetected(aCells[i]))
            return tinyfd_arrayDialog("INVALID ROWS WITH QUOTES", 0, NULL, 0, NULL);
    }
    ...
    char const* const* aOptions)
{
    ...
    if (tfd_quoteDetected(aTitle))
        return tinyfd_checklistDialog("INVALID TITLE WITH QUOTES", aNumOfOptions, aOptions);
    for (i = 0; i < aNumOfOptions; i++)
    {
        if (tfd_quoteDetected(aOptions[i]))
            return tinyfd_checklistDialog("INVALID COLUMNS WITH QUOTES", 0, NULL);
    }
